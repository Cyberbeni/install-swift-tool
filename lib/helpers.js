...
function getUuid(url, commitHash) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        switch (os.platform()) {
            case "darwin": {
                let macVersion = yield exec('sw_vers', ['-productVersion']);
            ...
            case "linux": {
                const osCodename = yield exec('lsb_release', ['-c']);
            ...
            default: {
                const osVersion = yield exec('uname', ['-v']); // os.version is somehow undefined on GitHub runner
                const swiftVersion = yield exec('swift', ['-version']);
                additionalInfo = `${osVersion}-${swiftVersion}`;
            }
        }
        core.info(additionalInfo);
        return uuid_1.v5(`${url}-${commitHash}-${additionalInfo}`, '6050636b-7499-41d4-b9c6-756aff9856d0');
    });
}
